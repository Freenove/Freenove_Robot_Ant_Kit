...
void Emotion_Show(void)
{
  if (emotion_task_mode == 0)
    ...
    wheel(2, 100);
  else if (emotion_task_mode == 5)
    wheel(1, 100);
  else if (emotion_task_mode == 6)
    eyesBlink(100);
}

//Emotion set mode
void Emotion_SetMode(int mode, int static_emotion)
{
  if (mode <= 6) {
  ...
  else if (mode == 7)
  {
    ...
    emotion_task_mode = 7;
    int emotion_flag = random(0, 22);
    staticEmtions(emotion_flag);
  }
  else if (mode >= 8)
  {
    emotion_task_mode = 7;
    int emotion_flag = static_emotion;
    staticEmtions(emotion_flag);
  }
...
byte static_emotion_left[][8] =
{
  ...
  0x04, 0xA, 0x11, 0x10, 0x08, 0x04, 0x02, 0x01,
...
byte static_emotion_right[][8] =
{
  ...
  0x20, 0x50, 0x88, 0x08, 0x10, 0x20, 0x40, 0x80,
